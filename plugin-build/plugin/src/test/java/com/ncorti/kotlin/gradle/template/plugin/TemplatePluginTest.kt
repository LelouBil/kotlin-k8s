...
class TemplatePluginTest {
    ...
    fun `plugin is applied correctly to the project`() {
        ...
        assert(project.tasks.getByName("templateExample") is GenerateCRDsTask)
    }
    ...
    fun `parameters are passed correctly from extension to task`() {
        ...
        (project.extensions.getByName("templateExampleConfig") as KotlinK8sExtension).apply {
            tag.set("a-sample-tag")
            message.set("just-a-message")
            outputFile.set(aFile)
        }

        val task = project.tasks.getByName("templateExample") as GenerateCRDsTask
